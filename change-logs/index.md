...
> 示例场景：
> - 【销售合同】及其从应用【订单明细】，在某些状态下，不允许新增、修改或删除
>   - 销售合同在“已收款”状态后，不能再新增、修改、删除订单明细
>   - 销售合同在“已审批”之前，是不能新增开票信息的

在这个版本中，业务流程的锁定配置支持设置当前应用及从应用数据锁定范围。当应用处于锁定状态时，可以将子应用一起锁定，并限制指定从应用新增数据。你可以在业务流程的锁定配置中进行设置：
![](https://swstatic.saleswork.cn/docs/changelog/ywlcsdpz1112.jpg)

如果钩上图中1处的配置，处于此状态时，计算、汇总等字段将参与计算，其值保持不变，常用于数据归档等场景。当不钩上时，即使当前数据处于锁定状态，其从应用发生变化时仍可能会导致当前数据的计算、汇总字段发生变化。

图中2处用于配置处于此状态时，是否限制其从应用的新增操作。如图所示，当“销售合同”处于这个状态时，将不能新增“服务单”和“开票”。
...
![](https://swstatic.saleswork.cn/docs/changelog/20gu1z.png)
...
![](//swstatic.saleswork.cn/docs/changelog/2023/psa-changelog-2024-03.png)
...
![](//swstatic.saleswork.cn/docs/changelog/2023/psa-changelog-2024-09.png)
